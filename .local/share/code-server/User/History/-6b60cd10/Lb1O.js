...
// Thunk to fetch a random recipe from the API
...
const randomRecipeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      // Handle POST request for saving the recipe
      ...
      .addCase(fetchRandomRecipe.fulfilled, (state, action) => {
        ...
        const ingredients = getFormattedIngredients(meal); // Concatenate ingredients and quantities
...
const getFormattedIngredients = (meal) => {
  let ingredients = [];
  for (let i = 1; i <= 20; i++) {
